...
export default function Scorecard({ data }: { data: ReportData }) {
  const top3 = data.competitors
    .filter((c) => !c.isSubject)
    .slice(0, 3);
  const summaryLine = data.executiveSummary.overview.split(". ").slice(0, 2).join(". ") + ".";
  ...
  return (
    <div className="bg-surface rounded-none sm:rounded-lg border border-border overflow-hidden shadow-sm">
      ...
      <div className="px-6 py-6 border-b border-border">
        ...
        <p className="text-sm text-secondary leading-relaxed">{summaryLine}</p>
      </div>
      ...
      {topRec && (
        <div className="px-6 py-6">
          ...
          <div className="bg-light-bg rounded-lg p-4 border border-border-subtle">
            <p className="text-sm font-semibold text-dark mb-1">{topRec.title}</p>
            <p className="text-xs text-secondary leading-relaxed">{topRec.description}</p>
          </div>
        </div>
      )}
    </div>
